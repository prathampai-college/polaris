...
| AI SNN | snnTorch LIF `5→32→16→1` event-gated 0.8mW idle vs 8.2mW ANN 90% saved — `snn_weights.json` (`model: linear-proxy` honest) + JS fallback cached residual | ₹0 | No neuromorphic chip/Python at edge; <200ms, `snn_verify.mjs` real ONNX `1546B p50 ~0.08ms` 6 checks. |
