...
| **II Neuromorphic SNN** | Continuous dense ANN 8.2mW cloud | snnTorch LIF `5→32→16→1` rate `T=20` event-gated `Δ>0.12` — 0.8mW idle (90% saved) `shared/src/power.ts:1` | `ai/snn/encoder.py:1` `ai/snn/train_snn.py:1` `hq/app/snn_forecast.py:1` `field/lib/snn/engine.ts:1` |
| **III DTN Data Muling** | Continuous REST — uplink default drops | `dtn_bundles` custody + `BroadcastChannel`/QR mule + LWW+VC `compare/merge` | `shared/src/dtn/vector_clock.ts:1` `shared/src/dtn/bundle.ts:1` `field/lib/dtn/mule.ts:1` `hq/app/dtn.py:1` |
...
- **Offline DB:** SQLite WASM OPFS/WAL (`@sqlite.org/sqlite-wasm`) — single `polaris.db` with `outbox`/`dedupe`/`sync_state`/`vessels`/`dtn_bundles`/`asset_positions`/`snn_state`. Schema `shared/sql/schema.sql:1` → `field/lib/db.ts:18` inline mirror + `hq/app/db.py:118` (strips PRAGMA/BLOB→BYTEA for PG). VC cols `vector_clock TEXT` backfilled via `ALTER TABLE ADD COLUMN` in `_ensure_dtn_sqlite` `hq/app/db.py:115`.
