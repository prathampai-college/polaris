...
| **Offline DB** | `@sqlite.org/sqlite-wasm` 3.53, OPFS/WAL | WAL mode survives crashes; `outbox` (incl. `BUNDLED`), `dedupe`, `sync_state.vector_clock`, `vessels`, `dtn_bundles`, `asset_positions`, `snn_state` live in one `polaris.db`. `BEGIN IMMEDIATE` (`field/lib/db.ts:113`) + `psycopg_pool 4/20` on HQ (`hq/app/db.py:_get_pool`). Indexed `outbox(status,created_at)`, `vessels(station_id)`, `dtn_bundles(dst,created_at)`, `asset_positions(station_id)`. |
| **Sync engine + DTN** | Node `ws` 8.17 + `@msgpack/msgpack` 3.0 + `ulid` 2.3 + CRC32 + AES-GCM + VC + `MAX_WIRE_SIZE` | Wire 70–80% smaller (patch-only 95.6% saving `shared/test/codec.test.mjs`). Every update ULID+VC, `sizeReport` logs saving, retries `SENT`→`ACKED` and **`BUNDLED` re-tried on reconnect** (`field/lib/sync.ts:126` `PENDING|SENT|BUNDLED` drain, `shared/src/types.ts:10` `BUNDLED`), `PING/PONG` 30s, `>2KB→FAILED` ack (`gateway.ts:6` `shared/src/wire.ts:1`). Downstream `assets|indents|vessels` incl. bundles in `SYNC_INIT_RESP` (`sync.ts:55`), `BroadcastChannel('polaris-mule')` + `bundleToBase64` QR (`bundle.ts:1`). |
| **AI — thermo hybrid + SNN** | `onnxruntime-node` 1.17, ONNX <2MB (`ai/thermo_residual.onnx` 2.0KB single-file) + `snn_weights.json` + `thermo_snn.onnx` + acoustic | Hybrid `load_physics(station_id)` + residual `<200ms`, fallback `5*dg+0.3crew-2` (`forecast.py:66`). **SNN** snnTorch LIF `5→32→16→1` rate `T=20` (`train_snn.py:1` `encoder.py:1`), HQ numpy + JS mirrors (`snn_forecast.py:1` `engine.ts:1`, bit-exact vs ONNX diff <1e-6) gated `0.12` (`snn-config.ts:1`, generated via `scripts/export_snn_config.py`) — **residual cached on gate** (`_last_residual`), **`model: lif-5-32-16-1`** (`rmse_snn 6.52 vs rmse_lin 11.05`, `thermo_snn.onnx` 375KB p50 ~0.6ms). Watts pill spike-proportional `saved_pct` + `saved_pct_source` (`TodayTab`). |
| **Local tracking** | `shared/src/local_map.ts` 40×40 2m + `sim_lidar.ts` 360pts + `fusion.ts` Kalman `q0.01 r0.5` | LiDAR active in whiteout, camera blind. `fuse()` 70/30 + per-axis Kalman → conf 0.75 LiDAR alone ~0.79 fused, err <0.8m (`tracking_verify.mjs:1`), no GPS. **SIM-LIDAR badge** (`SourceBadge sim` `LocateTab`) + header `// Sim-only` (`sim_lidar.ts:1`) makes simulation explicit — no hardware required. |
| **HQ** | FastAPI + `psycopg[binary]` **`psycopg_pool 4/20`** + TimescaleDB (`timescale/timescaledb:latest-pg15` in Docker), SQLite WAL fallback (`hq/app/hq.db`) | Append-only audit, RBAC (`auth.py:1`), Timescale `telemetry` hypertable, `procurement_targets/physics_params/vessels/dtn_bundles/asset_positions/snn_state` (`schema.sql:106`). **Pool** `hq/app/db.py:_get_pool` + `release_conn` in `_fetch_all` (`main.py`), **async gateway notify** `httpx 1s non-blocking` (`main.py:_notify_gateway_async`, `create_task`/thread). Pollers 15m. |
...
LIVE_WEATHER_ENABLED=true     # master switch for live weather (Open-Meteo free/keyless) — false forces sim
...
```powershell
docker compose up --build   # field :3000 hq-dashboard :3001 gateway :8787 hq :8000 db :5432 pgdata
# LAN tablets: NEXT_PUBLIC_HQ_URL=http://<LAN-IP>:8000 NEXT_PUBLIC_GATEWAY_URL=ws://<LAN-IP>:8787 docker compose up --build  (ARG baked) or rely on runtime hostname fallback
# air-gapped: disconnect WiFi, docker compose up still works, PWA Workbox cached
...
| `NEXT_PUBLIC_HQ_URL` | Field, HQ dashboard (`hq-dashboard/app/page.tsx:5`, `field/Dockerfile ARG`, `hq-dashboard/Dockerfile ARG`) | `http://localhost:8000` | Build-time `ARG` + runtime `window.location.hostname` fallback + `/api/config` (`field/app/api/config`, `hq-dashboard/app/api/config`) — pass `NEXT_PUBLIC_HQ_URL=http://<LAN-IP>:8000` at `docker compose build` or rely on LAN fallback. Field fetches `GET /forecast/:id` + `GET /forecast/snn/:id` (`model` field). |
...
| `LIVE_AIS_ENABLED` / `AIS_ENABLED` | HQ (`hq/app/vessel_poller.py:15`) | `false` | Master switch for live AIS. When `false`, the schedule interpolation is used even if `AIS_API_KEY` is set; set it to `true` for live AISHub fetches. Every poll records a machine-readable `reason` (`ok` \| `no_key` \| `429` \| `forced_mock`) served at `GET /vessels/sources`. |
...
- **Today tab** (`field/components/tabs/TodayTab.tsx`). Shows forecast days-to-stockout with confidence intervals from `GET /forecast/ST-BHARATI` (`field/app/page.tsx:145`), plus the **SNN watts pill** (`0.8 mW vs 8.2 mW`, about 90% saved) from `GET /forecast/snn/:station` — active with a spike count when the network fires, idle when event-gated. A **freshness badge** (`LIVE WX · Ns ago` / `STALE` / `OFFLINE`, `field/components/SourceBadge.tsx:1`) rides on the telemetry line, fed by `source`/`fetched_at`/`age_sec` on the forecast payload. It also counts critical low stock (`qty ≤ 5` and `CRITICAL`) and soon-to-expire items (under 30 days, via `@shared/expiry.js`), and offers three telemetry buttons that post to `/telemetry`: calm (−15°C, 5 m/s → ~42 days), blizzard (−38°C, 22 m/s → ~18 days), and acoustic anomaly (0.95 → auto-orders bearings). A vessel ETA pill comes from the offline `listVessels()` (`field/lib/db.ts:296`, fed by `DOWNSTREAM_DELTA vessels`).
...
- **Indent Workbench.** The indent table (`hq-dashboard/app/page.tsx:808`) shows a short ID, SKU, quantity, urgency pill, and status pill (amber `DRAFT`, teal `APPROVED`, purple `DISPATCHED`, emerald `RECEIVED`), plus a vessel pill once an indent is `DISPATCHED` with a `vessel_imo` (`hq/app/main.py:263`). Approvals go through `PATCH /indents/{id}` (`hq-dashboard/app/page.tsx:250`), which auto-attaches a `vessel_imo` from `GET /vessels?station_id` on dispatch. The `GET /indents` join includes `vessel_imo`.
...
- **3D Twin + Vessel Tracker** (`hq-dashboard/app/page.tsx:1013`). `VesselMap` (`hq-dashboard/components/VesselMap.tsx:1`) is a Leaflet `MapContainer` with an OpenStreetMap tile layer that first probes tile reachability (`fetch HEAD 0/0/0.png`, 2 s timeout). When the probe fails or the browser is offline, it renders a schematic with an ETA pill, captioned `Offline schematic — ETA pill primary when tiles unavailable` with a `SOG … · offline schematic` footer — so the view always works air-gapped. A shared **`SourceBadge`** (`LIVE AIS · Ns ago` / `STALE · Nh ago` / `MOCK SCHEDULE` / `OFFLINE`, `hq-dashboard/components/SourceBadge.tsx:1`) heads the tracker and every vessel row, fed by `source`/`fetched_at`/`age_sec`/`reason` on `GET /vessels`. The vessel table (`imo`, position, speed, ETA, source) comes from `GET /vessels?station_id`.
- **Telemetry Sources.** `GET /telemetry/sources` reports poller health (coordinates, source setting, `live_enabled`, `imd_status`, `fetched_at`, `age_sec`, last poll); `GET /telemetry/latest` and `GET /forecast/:station` annotate the telemetry payload with `source: live|stale_cache`, `fetched_at`, and `age_sec` for the badges; `GET /vessels/sources` reports `mode`, `ais_status`, `fetched_at`, `age_sec`, and `reason`. `GET /physics/{station}` (`hq/app/main.py:499`) reports per-station coefficients; `GET /tracking/positions` (`hq/app/main.py:903`) reports local-frame positions; `GET /dtn/bundles` (`hq/app/main.py:844`) and `GET /dtn/conflicts` (`hq/app/main.py:854`) report DTN state.
...
The field `SyncWorker` (`field/lib/sync.ts:13`) runs a full-duplex loop. `connect()` sends encrypted `SYNC_INIT`; `drain()` every 2s sends up to 20 `PENDING|SENT|BUNDLED` rows (`BUNDLED` re-tried on reconnect, `shared/src/types.ts:10`), `draining` guard prevents overlap, `sizeReport` logs patch-only 95.6% saving (`shared/test/codec.test.mjs`). When socket closed, rows bundle via `createAndSaveMuleBundle` (`field/lib/dtn/mule.ts:1`) → `BUNDLED` custody `BroadcastChannel('polaris-mule')` QR. On reconnect `pushBundlesToHQ()` + WS drain flush `BUNDLED` → `ACKED`. Incoming by type: `DOWNSTREAM_DELTA` (indents `db.ts:191`, vessels `db.ts:278`, assets VC merge), `SYNC_INIT_RESP` reconciles indents+bundles, ACKs (`APPLIED|DEDUPED|CONFLICT_CRITICAL|APPLIED_LOCAL_WINS|FAILED`) merge VC (`shared/src/dtn/vector_clock.ts:1`). Gateway (`gateway.ts:55`) bridges WS + HQ `/sync/ingest` + `/dtn/exchange` + `POST /internal/broadcast_delta` (`X-PSK` strict, 401 if missing, `readJson()` deduped). HQ uses `psycopg_pool 4/20` (`hq/app/db.py`) + async `httpx` notify (`main.py:_notify_gateway_async`) so gateway delay never blocks FastAPI.
...
**Neuromorphic SNN (event-driven):** `ai/snn/encoder.py:1` sigmoid→rate→Poisson `T=20` (`to_spike_train()` loads `scaler_snn.json` first, legacy warns); `ai/snn/train_snn.py:1` trains snnTorch LIF `5→32→16→1` (`beta 0.9 threshold 1.0`, BPTT + atan surrogate, standardized targets) exports `snn_weights.json` (`model: lif-5-32-16-1`, `rmse_snn 6.52 vs rmse_lin 11.05`, layers + `y_mean/y_std` + `spike_activity`) + `scaler_snn.json` + `thermo_snn.onnx` (375KB single-file, never placeholder). HQ `hq/app/snn_forecast.py:1` (`predict_snn_total()`) runs a numpy LIF mirror bit-exact vs ONNX (diff <1e-6; one-step-delayed subtract reset per snnTorch 1.0 `reset_delay=True`) event-gated — `|Δnorm|<0.12` → idle **residual = cached `_last_residual` (not zero)** + `model` field — returns physics/residual/total/active/spike/model. Field JS mirror (`field/lib/snn/engine.ts:1`, matrices baked into `shared/src/snn-config.ts` via `scripts/export_snn_config.py`) same dynamics (verified vs ONNX to 4e-5) + gate + cached residual + `model`. Energy via `GET /forecast/snn` `saved_pct` + `saved_pct_source` (spike-proportional estimate, sim-only) → watts pill. Bench `scripts/snn_verify.mjs` real ONNX `input→output p50 ~0.6ms <200ms`, fails on `linear-proxy` regression.
...
One honest caveat: physics-informed forecast, CI is ±15% placeholder (`days*0.85/1.15`, labeled `ci_source: placeholder_15pct`) until 30d live burn lands via `scripts/calibrate_physics.py`; **SNN is now real `lif-5-32-16-1`** (trained on synthetic CSV until live burn arrives — fine-tune with `python ai/snn/train_snn.py` once metered); `dg_load` is synthetic sine (`dg_source: synthetic`) until `DG_SOURCE=meter` + flow-meter bridge.
...
`docker-compose.yml:46` passes `AIS_API_KEY`, `VESSEL_MODE`, `VESSEL_POLL_SEC`, plus `LIVE_WEATHER_ENABLED` (default `true`) and `LIVE_AIS_ENABLED` (default `false`) through. The vessel poller runs a boot poll ~5s after startup (after a short settle so `/health` answers first) so first positions land seconds after HQ boot instead of after a full 15-minute interval. Local development without a key uses the 3-vessel schedule interpolation (`hq/app/vessel_poller.py:25`), verifiable at `GET /vessels?station_id=ST-BHARATI` with `source: stale_cache|mock` plus `fetched_at`, `age_sec`, and `reason`.
...
|-------|----------------|
| **Simulated LiDAR** (`field/lib/sensors/sim_lidar.ts:1`) | 360pt scan every 3s `polarToCart` (`shared/src/local_map.ts:1`) 0.12m noise, `r=min(rBase,vis)`, hit-tested C1–C6. **SIM-LIDAR badge** (`SourceBadge sim` `LocateTab`) + `// Sim-only` header — no hardware. |
...
| **Fusion** (`field/lib/sensors/fusion.ts:1`, `fuse()` `shared/src/local_map.ts:1`) | 70% LiDAR +30% camera → per-axis `Kalman1D q0.01 r0.5` (ponytail: 2D EKF if needed), LiDAR alone 0.75 fused ~0.79. |
...
| **UI** (`field/components/tabs/LocateTab.tsx:1`) | The `LOCAL` mode shows the 40×40 grid (2 m cells) on one dpr-aware `<canvas>` with fused dots (teal ≥60% conf, amber below; gold origin); the `GPS` mode shows a red “GPS Unavailable — ionospheric whiteout” card. `startFusionLoop()` (`field/lib/sensors/fusion.ts:1`) injects a 15% whiteout chance at 0.8 m visibility, proving the LiDAR carries tracking while the camera is blind. Error stays under 0.8 m (`scripts/tracking_verify.mjs:1`). |
...
- **UI hardening.** Both apps wrap their root layout plus the 3D twin and vessel map in `<ErrorBoundary>` retry cards (`field/components/ErrorBoundary.tsx:1`, `hq-dashboard/components/ErrorBoundary.tsx:1`) — a WebGL/Leaflet crash degrades to one card, never a blank app. Both `Container3D` twins dispose geometries/materials on unmount (`GLCleanup`) with capped `dpr` and `low-power` GL preference, preventing tablet context loss. The theme is **polar-night + aurora**: night `#060A13` surfaces, aurora teal `#2DD4BF` primary actions/AI, ice cyan info, amber stale/mock/warn, emerald live/ok, red critical-only — with shared `.pill-live/.pill-stale/.pill-critical/.pill-ai` utilities in both `globals.css`.
...
- **Live vessels.** The 15-minute adaptive poller (boot poll ~5s after startup, positions seconds after boot) degrades gracefully on `429`s to schedule interpolation plus cache; `GET /vessels?station_id` always answers with `source: live|stale_cache|mock`, `fetched_at`, `age_sec`, and `reason`, and `PATCH /indents {vessel_imo}` validates the vessel as a foreign key.
...
**Air-gapped:** `docker compose up` works WiFi off (PWA Workbox cached, Leaflet schematic ETA pill, `/tmp/ais_cache.json` vessel fallback, `dtn_bundles` WAL custody, `BUNDLED` re-tried on reconnect, pool+async keep HQ responsive under satellite latency). Build `ARG`s ensure LAN IP baked, fallback via `window.location.hostname` if stale.
...
| `GET /vessels` is `[]` or always `source:mock`/`stale_cache` | No AIS key, failed probe, or offline | Check `GET /vessels/sources` for the `reason` (`no_key`, `429`, `forced_mock`): set `AIS_API_KEY` + `LIVE_AIS_ENABLED=true` for live data, or accept schedule interpolation — the badge shows its age. The `VesselMap` shows the schematic + ETA pill when tiles are unreachable. |
