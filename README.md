...
- **Live vessels.** The 15-minute adaptive poller (boot poll on startup, ~15s to first positions) degrades gracefully on `429`s to schedule interpolation plus cache; `GET /vessels?station_id` always answers with `source: live|stale_cache|mock`, `fetched_at`, `age_sec`, and `reason`, and `PATCH /indents {vessel_imo}` validates the vessel as a foreign key.
