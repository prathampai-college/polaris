...
| **AI — thermo hybrid + SNN** | `onnxruntime-node` 1.17, int8 ONNX <2MB (`ai/thermo_residual.onnx` 1.3KB) + `snn_weights.json` + `thermo_snn.onnx` + acoustic | Hybrid `load_physics(station_id)` + residual `<200ms`, fallback `5*dg+0.3crew-2` (`forecast.py:66`). **SNN** snnTorch LIF `5→32→16→1` rate `T=20` (`train_snn.py:1` `encoder.py:1`), JS mirror (`engine.ts:1`) gated `0.12` (`snn-config.ts:1`) — **residual cached on gate** (`_last_residual` `snn_forecast.py:53`), **`scaler_snn.json` first** (`encoder.py:6`), **real ONNX bench** `snn_verify.mjs` (`input→output 1546B p50~0.08ms`), **`model: linear-proxy` honest** until LIF matrices (pill tooltip `linear-proxy` vs `LIF 5→32→16→1`). Watts pill `0.8mW idle 0.82mW active vs 8.2mW ANN`. |
...
**Neuromorphic SNN (event-driven):** `ai/snn/encoder.py:1` sigmoid→rate→Poisson `T=20` (`to_spike_train()` loads `scaler_snn.json` first, legacy warns); `ai/snn/train_snn.py:1` trains snnTorch LIF `5→32→16→1` (`beta 0.9 threshold 1.0`) exports `snn_weights.json` (`model: linear-proxy` honest until LIF matrices) + `scaler_snn.json` + `thermo_snn.onnx` (validated 1546B, never placeholder). HQ `hq/app/snn_forecast.py:1` (`predict_snn_total()`) event-gated — `|Δnorm|<0.12` → idle **residual = cached `_last_residual` (not zero)** + `model` field — returns physics/residual/total/active/spike/model. Field JS mirror (`field/lib/snn/engine.ts:1`) same gate + cached residual + `model`. Power `ANN 8.2mW → SNN 0.8mW idle /0.82mW active ~90%` via `GET /forecast/snn` `saved_pct/model` → watts pill (`TodayTab` tooltip `linear-proxy` vs `LIF 5→32→16→1`, `field/app/page.tsx:585`). Bench `scripts/snn_verify.mjs` real ONNX `input→output p50 ~0.08ms <200ms`.
...
One honest caveat: physics-informed forecast, CI is ±15% placeholder (`days*0.85/1.15`) until calibration firms; **SNN is `linear-proxy` honestly labeled** until full LIF `5→32→16→1` ONNX graph ships — pill shows model in tooltip.
