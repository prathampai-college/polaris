...
> **Live demo 3.5 min:** `PITCH_DECK.md`. Field PWA + Sync Gateway + HQ Dashboard run via `docker compose up` with WiFi off. **Production-ready:** real DB-driven procurement, per-station physics, live Open-Meteo weather, AIS vessel tracking with mock fallback, Leaflet vessel map, bulk inventory import. No hardcoded mocks.
...
4. [System Capabilities — All Real Data](#system-capabilities--all-real-data)
...
11. [Vessel Tracking — AIS Adaptive](#vessel-tracking--ais-adaptive)
12. [Security & Resilience](#security--resilience)
13. [Testing — Unit / Chaos / Budgets](#testing--unit--chaos--budgets)
14. [Project Structure](#project-structure)
15. [Deployment — Production Ready](#deployment--production-ready)
16. [Troubleshooting](#troubleshooting)
17. [Feasibility & Pitch](#feasibility--pitch)
...
│  vessels (offline ETA)      │  ◄── Vessel DOWNSTREAM_DELTA ──────────  │  + Vessel Poller (AIS)     │
└────────────┬────────────────┘                                           └──────────────┬───────────────┘
             │ vessels cache / vessel_schedule.json mock fallback                    │
             │ telemetry poller Open-Meteo/IMD (15m)  vessels poller AISHub (15m) ──┘
             └────────── Thermo Hybrid (physics + ML residual per station) ───────────┘
```

**One language on field live path:** TypeScript/Node (`field` + `sync-gateway` + ONNX runner) — zero cross-FFI at the edge. HQ and training stay Python where they belong.

- **Field reads:** local SQLite WASM OPFS `polaris.db` (WAL, `shared/sql/schema.sql:1` mirrors `field/lib/db.ts:9`). `getDb()` `field/lib/db.ts:33` falls back to `:memory:` with `window.__polaris_ephemeral` warning if `SecureContext` missing. `listVessels()` `field/lib/db.ts:293` shows offline vessel ETA from `DOWNSTREAM_DELTA vessels`.
- **HQ reads:** `GET /assets` `hq/app/main.py:193` joins `assets→crates→containers` to return `station_id, container_id` — client scopes via `station_id` not hardcoded filters. `GET /vessels` `hq/app/main.py:517` returns live AIS or mock schedule with `source:live|mock`.
- **Trends:** `GET /telemetry/history` `hq/app/main.py:351` aggregated; `TrendChart` `hq-dashboard/components/TrendChart.tsx:28` shows empty state unless `?demo=1`. `GET /telemetry/sources` `hq/app/main.py:354` health. `GET /physics/{station}` `hq/app/main.py:499` per-station `K1/K2/K3`.
...
|-----------|-------|----------------------|
| **Field PWA** | Next.js 14.2.5 + Tailwind + Three.js `@react-three/fiber/drei` 3D X-Ray + `html5-qrcode` 2.3.8 + Workbox | QR offline, 3D crate locator (shared `CONTAINER_SPECS` via `@polaris/shared`), glove 48px hit targets, tactical dark, `fontLarge` 200% toggle. Split into `Icons` + `tabs/{Today,Inventory,Scan,Indents,Locate}`. `TodayTab` shows `forecast.days_to_stockout + ci` from `GET /forecast/:id`. |
| **Offline DB** | `@sqlite.org/sqlite-wasm` 3.53 OPFS/WAL | WAL crash safety, `outbox`/`dedupe`/`sync_state`/`vessels` in same DB, `BEGIN IMMEDIATE` `field/lib/db.ts:113` avoids TOCTOU. Indexes `idx_outbox_status`, `idx_vessels_station`. |
| **Sync Engine** | Node `ws` 8.17 + `@msgpack/msgpack` 3.0 + `ulid` 2.3 + CRC32 + AES-GCM (Node `crypto` / WebCrypto) | 70–80% smaller than JSON, idempotent `ulid` replay, `sizeReport` logging, `SENT` retry on next `drain` every 2s with `draining` guard `field/lib/sync.ts:13`, `PING/PONG` 30s, `>2KB` returns `FAILED` not silent drop `sync-gateway/src/gateway.ts:6`. Handles `DOWNSTREAM_DELTA` for `assets`, `indents`, `vessels` `field/lib/sync.ts:55`. |
| **AI** | `onnxruntime-node` 1.17 int8 <2MB `ai/thermo_residual.onnx` (1.3KB) + `ai/scaler.json` + Acoustic Prognostics | Hybrid `phys + ML residual` per-station `hq/app/forecast.py:7` `load_physics(station_id)`, `<200ms`, fallback `5*dg+0.3*crew-2` `hq/app/forecast.py:66`. |
| **HQ** | FastAPI + `psycopg[binary]` + TimescaleDB `timescale/timescaledb:latest-pg15` (Docker) / SQLite WAL fallback `hq/app/hq.db` | Audit append-only, RBAC `hq/app/auth.py:1`, Timescale hypertable for `telemetry`, `procurement_targets` + `physics_params` + `vessels` `shared/sql/schema.sql:106`. Pollers `telemetry_poller.py` + `vessel_poller.py` (15m). |
| **HQ Dashboard** | Next.js 14 (`:3001`) + Recharts 3.10 + Three.js 0.185 + **Leaflet 1.9.4 / react-leaflet 4.2.1** | Fleet view, forecast 42→18d, SSE live `EventSource /telemetry/stream` `hq-dashboard/app/page.tsx:180`, `TrendChart` empty-state `hq-dashboard/components/TrendChart.tsx:28`, `ProcurementTable` DB-driven `hq/app/main.py:520`, `VesselMap` `hq-dashboard/components/VesselMap.tsx:1` Leaflet overlay with offline ETA pill fallback (air-gapped tiles → schematic). |
| **Training** | Python PyTorch + ONNX export `ai/training/generate.py` + `train.py` | 1095-row synthetic physics+noise ships only `.onnx`; production uses per-station `physics_params` calibration `scripts/calibrate_physics.py` `np.linalg.lstsq` on 30d `telemetry` + `transactions` burn. |

---

## Data Model — Single Source of Truth

`shared/sql/schema.sql:1` is canonical DDL (copied to `field/lib/db.ts:9` inline mirror for browser bundle; `hq/app/db.py:45` strips `PRAGMA` and `BLOB→BYTEA` for PG). HQ `hq/app/db.py:118` handles Timescale extension + per-statement execute + seed iff `COUNT(*)=0` else migrates `procurement_targets`, `physics_params`, `vessels`.
...
| `assets` | `id`, `sku UNIQUE` → `crates` | `category` FUEL_DIESEL…SCIENTIFIC, `qty REAL`, `unit`, `expiry_date`, `criticality` CRITICAL/HIGH/LOW, `barcode`, `version` | 20 SKUs A1–A20 (Diesel 4200L C1-K1, O2 24cyl C2-K1 exp 2026-09-15) |
...
| `vessels` | `imo PK` → `stations` | `name`, `lat`, `lon`, `sog`, `eta`, `station_id`, `last_seen` — live AIS or mock schedule `shared/vessel_schedule.json:2` (Sagar Nidhi) | 3 mock vessels `hq/app/vessel_poller.py:25` |
...
| `dedupe` | `ulid PK` | idempotency `processed_at` | every `POST /sync/ingest` |

Indexes: `idx_assets_crate`, `idx_outbox_status (status, created_at)`, `idx_transactions_asset`, `idx_vessels_station`.

**Seed idempotency:** `hq/app/db.py:170 seed(cur)` `ON CONFLICT DO NOTHING` / `INSERT OR IGNORE` → `field/lib/db.ts:58 seedIfEmpty` same.
...
| **Station scoping** | `STATION_CRATES` hardcoded client filter | `GET /assets` `hq/app/main.py:193` returns `station_id, container_id` via `LEFT JOIN crates→containers`; client scopes via `station_id` | `hq/app/db.py:45` schema |
| **Trend** | 7-day dummy always | Empty dashed border "No telemetry yet — add `?demo=1`" unless `?demo=1` `hq-dashboard/components/TrendChart.tsx:47` | — |
...
**Production hardening in this release:** `_ensure_*_sqlite` migrations for old DBs, `TrendChart` deps fix, `field` `station_id` parity, `indents` 9-col (`vessel_imo`) explicit inserts (no 8-value crash), Leaflet offline fallback, `VESSEL_MODE auto` adaptive throttle.
...
AIS_API_KEY=                  # optional AISHub/MarineTraffic key; empty → vessel_schedule.json mock
...
# first boot seeds procurement_targets 3 rows + physics_params 3 rows + vessels (mock) + assets 20 SKUs if empty
```
...
| `HQ_URL` / `GATEWAY_INTERNAL_URL` | `field`, `gateway` `hq/app/main.py:52` | `http://localhost:8000` / `http://hq:8000` in Docker | Field → HQ HTTP; `hq` → `gateway` internal `POST /internal/broadcast_delta` with `X-PSK` `hq/app/main.py:52`. |
| `GATEWAY_PORT` | `gateway` | `8787` | WebSocket port `sync-gateway/src/gateway.ts:6`. |
| `NEXT_PUBLIC_HQ_URL` | `field`, `hq-dashboard` `hq-dashboard/app/page.tsx:5` | `http://localhost:8000` | Build-time Next.js public var — field `GET /forecast/:id`, HQ `Promise.all` 8 fetches `hq-dashboard/app/page.tsx:140`. |
...
| `TELEMETRY_SOURCE` | `hq` `hq/app/telemetry_poller.py:13` | `both` | `both\|openmeteo\|imd\|sim` — both tries IMD if key present else Open-Meteo free ( `https://api.open-meteo.com/v1/forecast` `hq/app/telemetry_poller.py:31` ); `sim` disables external poll (fixtures + `?demo` only). |
...
- **Today Tab** `field/components/tabs/TodayTab.tsx` — `forecast.days_to_stockout + ci` from `GET /forecast/ST-BHARATI` `field/app/page.tsx:145`, `criticalCount=qty≤5&CRITICAL`, `expiringCount<30d` via `@shared/expiry.js` `isExpiringSoon`, 3 telemetry buttons `POST /telemetry` (Calm `-15C/5m/s` → 42d, Blizzard `-38C/22m/s` → 18d, Acoustic `0.95`→ bearing indent). Vessel ETA pill from `listVessels()` `field/lib/db.ts:293` (offline `DOWNSTREAM_DELTA vessels`).
- **Inventory** `field/components/tabs/InventoryTab.tsx` — `listAssets()` `field/lib/db.ts:80` includes `station_id`. Search `sku+name+crate+category+barcode`, filter pills ALL/CRITICAL/EXPIRING/LOW≤3/FUEL/MEDICAL/SPARES. `consumeAsset` `field/lib/db.ts:113` `BEGIN IMMEDIATE` → `UPDATE assets + INSERT transactions/audit/outbox COMMIT`; expired `MEDICAL/OXYGEN/FOOD` blocked without `overrideExpired` + `CONSUME_OVERRIDE_EXPIRED` audit.
...
- **Indents** `field/components/tabs/IndentsTab.tsx` — `listIndents()` local (now includes `vessel_imo` `field/lib/db.ts:104`), `createIndent` `field/lib/db.ts:148` `INSERT indents DRAFT` + `outbox UPSERT msgpack`; strict `ALLOWED` `DRAFT→APPROVED→DISPATCHED→RECEIVED` `field/lib/db.ts:169`. `RECEIVED` with `vessel_imo` shows vessel ETA. Downstream applies via `applyDownstreamIndent` `field/lib/db.ts:191` / `applyDownstreamSyncInit` + `applyDownstreamVessel` `field/lib/db.ts:276` on `SYNC_INIT_RESP`.
- **Locate / 3D X-Ray** `field/components/tabs/LocateTab.tsx` + `field/components/Container3D.tsx` — `@polaris/shared/containers.js` `CONTAINER_SPECS`/`CRATE_COORDS` `shared/src/containers.ts:1` (6 containers, 12 crates), `OrbitControls`, wireframe envelope, color `CRITICAL qty≤5 red, ≤3 orange, HL gold #F59E0B`. Vessel list `listVessels()` shown alongside.

Sync drawer `field/app/page.tsx:585` shows `sent/acked/deduped/receivedDeltas/savingPct` `SyncWorker.stats` `field/lib/sync.ts:13` draining every 2s up to 20 rows `PING/PONG` keepalive, `CRC32`+`AES-GCM` via `shared/codec.web.ts`.
...
- **Thermo AI Forecast** — 3 burn cards `physics, residual, days` + same `TrendChart`/`ProcurementTable`.
...
- **Trends** `hq-dashboard/components/TrendChart.tsx:28` — 3 modes fuel/temp/load `AreaChart/LineChart` Recharts. Empty `trend` shows dashed empty state not fake 4700L; `?demo=1` restores 7-day sample.
...
- **Telemetry Sources** — `GET /telemetry/sources` `hq/app/main.py:354` poller health (coords, `TELEMETRY_SOURCE`, `last_poll`); `GET /physics/{station}` `hq/app/main.py:499` per-station `K1/K2/K3`.
...
| `GET` | `/assets` | `Asset[] {id, sku, name, category, qty, unit, expiry_date, criticality, crate_id, barcode, version, updated_at, station_id, container_id}` via join `hq/app/main.py:193` |
...
| `POST` | `/sync/ingest` | `DeltaFrame {ulid(26),device_id,entity,entity_id,op,patch,base_version,ts}` `hq/app/main.py:685` rate 600/min `hq/app/main.py:689`, dedupe, `CONFLICT_CRITICAL` if negative, `DEDUPED` on replay; `entity` includes `assets|indents|vessels` (indents `vessel_imo` handled) |
...
Field `SyncWorker` `field/lib/sync.ts:13` full-duplex: `connect()` sends `SYNC_INIT` wire, `drain()` every 2s sends `PENDING|SENT` up to 20 rows (retry until `ACKED`, `draining` guard), `onmessage` handles `DOWNSTREAM_DELTA`→`applyDownstreamIndent` `field/lib/db.ts:191` / `DOWNSTREAM_DELTA vessels`→`applyDownstreamVessel` `field/lib/db.ts:276` / `SYNC_INIT_RESP`→`applyDownstreamSyncInit`, `ACK APPLIED|DEDUPED|CONFLICT_CRITICAL|FAILED`. `sizeReport` logs `jsonBytes vs mpBytes saving 70-80%` `shared/src/codec.ts:toWire`. Gateway `sync-gateway/src/gateway.ts:55` bridges `wss.on connection` + `fetch HQ/indents?station_id` + `fetch HQ/sync/ingest` + `POST /internal/broadcast_delta` filtered by `station_id`, `MAX_WIRE_SIZE 2048` `shared/src/codec.ts`.
...
`scripts/calibrate_physics.py:1` per-station `np.linalg.lstsq` on 30d `AVG(tele) JOIN -SUM(qty_delta)` `total = BASE*(1+K1*(T_INSIDE-temp)+K2*wind)+K3*pd*BASE`, `UPDATE physics_params` `hq/app/db.py:90`.
...
- **Zero cloud:** `docker compose up` WiFi off, PWA Workbox pre-cached, OPFS `polaris.db` (ephemeral `:memory:` warned). Vessel cache `/tmp/ais_cache.json` persists fallback.
- **Transit:** TLS + AES-GCM per frame (`nonce 12B||tag 16B`) + `CRC32` framing, PSK per-station QR `64 hex` strictly validated (`sync-gateway/src/gateway.ts:6` `a*64` demo, `scripts/provision_station.mjs:1` generator), `X-PSK` on internal push `hq/app/main.py:52`, `DataView` byteOffset-safe.
...
- **Live Telemetry:** SSE `hq/app/main.py:363` `asyncio.Queue 100` + `field/app/page.tsx:180 EventSource` fallback 8s poll `hq-dashboard/app/page.tsx:180`; `GET /telemetry/sources` `hq/app/main.py:354` health. `GET /physics/{station}` `hq/app/main.py:499` per-station.
- **Live Vessels:** 15m poll adaptive, `429` graceful fallback to mock, cache, `GET /vessels?station_id` `hq/app/main.py:517`, `PATCH /indents {vessel_imo}` validated FK.
- **Audit:** immutable `audit_log` both field `field/lib/db.ts:132` + HQ `hq/app/main.py:244`, expiry override `CONSUME_OVERRIDE_EXPIRED`, replayable. `indents.vessel_imo` included in audit `before/after`.
- **DR:** WAL + `pg_dump` + `VACUUM INTO 'snapshot.db'`, Timescale source of truth, re-bootstrap via snapshot + delta replay. Vessel/Telemetry pollers restart via lifespan `hq/app/main.py:76`.
...
pytest hq/tests -v              # ingest, forecast 42/18d, indents lifecycle + vessel_imo, RBAC VIEWER, pessimistic lock — 11 passed

# quick (used in verify)
python -m pytest hq/tests -q    # 11 passed 7s (SQLite fallback, no Docker)
...
```powershell
# procurement_targets 3 rows, physics_params 3 rows, vessels 3 vessels
...
curl "http://localhost:3001/?demo=1" # vs plain /
```
...
shared/                 # @polaris/shared TS lib
  ...
  sql/schema.sql        # canonical DDL 14 tables (procurement_targets + physics_params + vessels) + idx_vessels_station
  seed.json / physics.json / vessel_schedule.json # Sagar Nidhi schedule mock fallback
...
field/                  # Next 14 PWA :3000 — Offline-first tablet
  ...
  lib/db.ts             # getDb() OPFS /polaris.db WAL + seedIfEmpty + consumeAsset BEGIN IMMEDIATE + createIndent + listAssets (station_id) + applyDownstreamVessel + listVessels + pullIndentsFromHQ (vessel_imo)
  lib/sync.ts           # SyncWorker ws://8787 drain every 2s, PENDING|SENT, downstream vessels
...
sync-gateway/           # Node ws :8787
  src/gateway.ts        # WSS + CRC/AES validation + POST /sync/ingest + broadcast_delta X-PSK (indents, assets, vessels)

hq/                     # FastAPI :8000 — Python 3.11
  app/main.py           # 823 lines: auth, assets (station_id join), indents (vessel_imo), stations/overview, forecast (per-station), procurement, telemetry, vessels, sync/ingest
  app/db.py             # init_db Postgres/SQLite, PROCUREMENT_SEED, physics_params seed, vessels migration _ensure_vessels_sqlite
...
hq-dashboard/           # Next 14 SOC :3001
  ...
  components/TrendChart.tsx # empty-state vs ?demo=1 dummy
...
All data planes are real — no mock fallbacks are synthetic at runtime except honest `?demo=1` for pitch:
...
First boot seeds `procurement_targets` 3 rows, `physics_params` 3 rows, `vessels` 3 mock (via poller 5s), assets 20 SKUs.

**Air-gapped:** `docker compose up` with WiFi off works; `field` PWA Workbox cached, `hq-dashboard` Leaflet falls back to schematic + ETA pill; `/tmp/ais_cache.json` persists vessel fallback.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `GET /assets` missing `station_id` | Old `hq/app/hq.db` without migration | `rm hq/app/hq.db*` + restart `uvicorn` — `init_db` `hq/app/db.py:118` recreates + seeds `procurement_targets`/`physics_params`/`vessels` + `vessel_imo` |
| `TrendChart` shows dummy 4700L with empty DB | Forgot `?demo=1` is now opt-in | Add `?demo=1` to URL for pitch sample, or `POST /telemetry` one row |
...
| `GET /vessels` `[]` or `source:mock` always | No AIS key / probe fails / offline | Normal — mock schedule `shared/vessel_schedule.json` is production fallback; set `AIS_API_KEY` for live, `VesselMap` shows ETA pill when tiles unavailable `hq-dashboard/components/VesselMap.tsx:44` |
...
Logs: `hq` `X-Request-ID` `hq/app/main.py:93`, `gateway` `sizeReport` JSON vs mp saving, `field` SyncDrawer `sent/acked/deduped`, `vessel_poller` `source:mock|live`.

---

## Feasibility

`COST_FEASIBILITY.md` — reuses existing rugged tablet + HQ VM + Iridium link, **₹0** new hardware, 2 days provision for 3 stations (`provision/ST-*.png` QR), `N` stations via `station_id` filter, Leaflet already installed `leaflet@1.9.4 react-leaflet@4.2.1` `hq-dashboard/package.json:14`, offline ETA pill needs no tiles.

## Pitch

`PITCH_DECK.md` — 3.5 min: Blizzard cut (msgpack 86% saving, dedupe) + Stockout 42→18d (ML toggle + fallback) + Vessel Sagar Nidhi mock→live (ETA pill) + arch (one TS language, Python HQ) + feasibility close. Add `?demo=1` to dashboard URL for the 7-day trend story when live DB empty.

## Compliance (§10)

`scripts/m5_verify.mjs` checks: air-gapped ✓, WAL ✓, budgets ✓ (10k 1.38MB, wire 231B), sync ✓, ML 1.3KB <2MB <200ms ✓, RBAC/audit/AES ✓, domain QR/indent/RBAC + vessel mock ✓, demo fallback ✓ (fake trend only with `?demo=1`, vessels `source:mock`).

---

## License

MIT — for NCPOR/MoES evaluation. Production-ready with no future-plan mocks; all feeds real with honest offline fallbacks.
