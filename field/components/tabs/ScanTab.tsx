  ...
  return (
    <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-4">
      <div className="card p-4 space-y-4">
        <div className="flex items-center justify-between"><div><h2 className="font-bold text-base text-white">QR / Barcode Scanner</h2><p className="text-xs text-white/50">Air-gapped offline camera decoding via html5-qrcode</p></div><button onClick={() => setShowQr((v: boolean) => !v)} className={`px-3.5 py-1.5 rounded-xl text-xs font-bold border transition ${showQr ? 'bg-white text-black border-white' : 'bg-teal-500 text-white border-teal-400 shadow-md shadow-teal-500/20'}`}>{showQr ? 'Close Camera' : '📷 Open Camera'}</button></div>
        {showQr && <QrWrap onScan={(text: string) => doScan(text)} onClose={() => setShowQr(false)} />}
        <div className="flex gap-2"><input value={scan} onChange={(e: any) => setScan(e.target.value)} onKeyDown={(e: any) => e.key === 'Enter' && doScan()} placeholder="Scan or type barcode SKU (e.g. FUEL-DIESEL-001)" className="flex-1 bg-black/40 border border-white/15 rounded-xl px-3.5 h-12 text-sm focus:outline-none focus:border-teal-400 transition" /><button onClick={() => doScan()} className="h-12 px-6 rounded-xl bg-teal-500 hover:bg-teal-400 text-white font-bold transition shadow-lg shadow-teal-500/25">GO</button></div>
        {selectedAsset && (
          <div className="rounded-2xl border border-teal-400/40 bg-blue-950/40 p-4 space-y-3 animate-in fade-in">
            <div className="flex items-center justify-between border-b border-teal-400/20 pb-2"><span className="text-[11px] font-mono tracking-widest text-teal-300 font-bold">SCAN RESULT • VERIFIED</span><span className="font-mono text-xs text-white/50">Crate {selectedAsset.crate_id}</span></div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => doConsume(selectedAsset.id, -1)} className="flex-1 h-11 rounded-xl bg-teal-500 hover:bg-teal-400 text-white font-bold text-xs transition">RESTOCK +1</button>
