    ...
    <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-4">
      <div className="card p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div><label className="text-xs font-semibold text-white/70 block mb-1">Target Asset</label><select value={indentAsset} onChange={(e) => setIndentAsset(e.target.value)} className="w-full bg-black/40 border border-white/15 rounded-xl px-3 h-11 text-xs focus:outline-none focus:border-teal-400 font-mono"><option value="">Select supply SKU…</option>{assets.map((a: any) => (<option key={a.id} value={a.id}>{a.sku} — {a.name} (Stock: {a.qty} {a.unit})</option>))}</select></div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div><label className="text-xs font-semibold text-white/70 block mb-1">Urgency</label><select value={indentUrg} onChange={(e) => setIndentUrg(e.target.value)} className="w-full bg-black/40 border border-white/15 rounded-xl px-3 h-11 text-xs font-bold focus:outline-none focus:border-teal-400"><option value="CRITICAL">🔴 CRITICAL (Immediate)</option><option value="MEDIUM">🟡 MEDIUM (Seasonal)</option><option value="LOW">🟢 LOW (Routine)</option></select></div>
          </div>
          ...
          {indents.map((ind: any) => {
            ...
            return (
              <div key={ind.id} className="bg-black/40 border border-white/10 rounded-xl p-3 space-y-2">
                <div className="flex items-center justify-between flex-wrap gap-2"><div className="flex items-center gap-2"><span className="font-mono text-xs font-bold text-white">{ind.id.slice(0, 8)}</span><span className="text-xs text-white/80 font-medium">{ind.sku || ind.asset_id} • <b>{ind.qty_requested}x</b></span>{isAuto && <span className="text-[9px] px-1.5 py-0.2 rounded bg-purple-500/20 text-purple-300 font-bold border border-purple-500/30">🤖 AI AUTOMATED</span>}</div><div className="flex items-center gap-2"><span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${ind.urgency === 'CRITICAL' ? 'bg-red-500 text-white' : 'bg-white/10 text-white/70'}`}>{ind.urgency}</span><span className="text-xs font-bold text-teal-300 font-mono">{ind.status}</span></div></div>
                <div className="grid grid-cols-4 gap-1 pt-1">{stages.map((stg, i) => (<div key={stg} className="flex flex-col gap-1"><div className={`h-1.5 rounded-full transition-all ${i <= stageIndex ? 'bg-teal-400' : 'bg-white/10'}`} /><span className="text-[9px] text-center font-mono text-white/40">{stg}</span></div>))}</div>
