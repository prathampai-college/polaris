  ...
  return (
    <div className="space-y-4">
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-5">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-mono tracking-widest text-teal-300 font-bold">THERMO HYBRID RESIDUAL AI</span>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-400/15 text-teal-300 font-bold border border-teal-400/25">{forecast.used_model ? 'ONNX int8 <2MB' : 'Physics Fallback'}</span>
              </div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <button onClick={() => setTab('scan')} className={`card-raised p-5 flex items-center gap-4 hover:bg-[#18294F] border border-white/10 hover:border-teal-400/40 transition text-left group ${glove ? 'py-7' : ''}`}><div className="w-12 h-12 rounded-2xl bg-teal-500 grid place-items-center text-white shadow-lg shadow-teal-500/30 shrink-0"><Icons.scan /></div><div><div className="font-bold text-base text-white group-hover:text-teal-300">Scan QR / Barcode</div><div className="text-xs text-white/50">Offline camera scan & instant 1-tap consumption</div></div><span className="ml-auto w-8 h-8 rounded-full bg-white/10 group-hover:bg-teal-500 group-hover:text-white grid place-items-center transition">→</span></button>
